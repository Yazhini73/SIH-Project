"""Reference-backed protein calibration for tables without paired images."""
...
def calibrate_protein(chamber: np.ndarray, rows: Iterable[dict[str, str]]) -> float:
    """Return the percentage from the nearest existing Color reference row."""
    ...
    for row in rows:
        try:
            ...
            continue
    if not candidates:
        raise ValueError("The protein reference table has no usable calibration rows.")
